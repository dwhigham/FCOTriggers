...
function auto_server_failover(p)
	if(p == nil) then
		return {
			...
			name = "Auto ServerFailover",
			description = "Starts a backup server in the event of a live server shutting down",
			priority = 0,
...
if(p.input:getJobType() == new("JobType","SHUTDOWN_SERVER")) then
	...
	local customerCheck = checkCustomerKey(customerUUID, "AUTO_SERVER_FAILOVER")
